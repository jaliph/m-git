...
func ConfigureGit() (err error) {
	err = exec.Command("git", "config", "--global", "user.name", "jaliph").Run()
	if err != nil {
		return
	}
	err = exec.Command("git", "config", "--global", "user.email", "akash.chandra.4u@gmail.com").Run()
	return
}
...
func RunMultiGit(command string, ignoreErrors bool, mgRoot string, mgRepos string, useConfigFile bool) (output string, err error) {
	out, err := exec.Command("which", "m-git").CombinedOutput()
	if err != nil {
		return
	}

	if len(out) == 0 {
		err = errors.New("m-git is not in the PATH")
		return
	}
	...
	env := os.Environ()
	if useConfigFile {
		configFile := path.Join(mgRoot, "m-git-test-config.toml")
	...
	cmd := exec.Command("m-git", components...)
	cmd.Env = env
